...
- Signal in the Signalize is basically a "variable" that you can watch for changes or when it's value is accessed.
- Every Signal you create while using Signalize is an instance the `Signal` class.
- The method `signal` creates a new instance of the `Signal` class with the value you pass in during the inicialization.

Let's start with how signal can be used:
```js
// 1. Creating a signal
...
Signals can be watched for the following events. See example bellow:
- **on every get** - to track places, where signal is used
- **before it is set** - usefull to check the new value and possibly modify the value or block setter.
- **after it is set** - trigger some callback like recalculation variables that depends on the signal

Signals are often used with the [bind method](/plugins/signalizejs-bind).
...
### Signal
Signal is a class that is used while creating a new signal with the `signal` method.
