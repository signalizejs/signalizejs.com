...
- If the only difference between the navigation URL and the current URL in `window.location` is a `hashtag`, it will attempt to select that element and **scroll it into the viewport**.
